            ...
            messages=[
                {"role": "system",
                 "content": "You are a helpful assistant. You will be provided with the concept from the user,"
                            " for which you generate similar but different concepts. Make sure that concepts generated by you are not the same as the original concept."},
                {"role": "user",
                 "content": "Generate a list of five different, but similar or related concepts. Focus on concepts that are close semantically but look and are different."
                            "The output provided by you should in the format of a list separated by semicolons. Don't write anything apart from the "
                            "list of concepts separated by semicolon. "
                            f"The concept in question: {concept}. Make sure that the concepts proposed by you are actually different than {concept}"}
            ]
        )
...
def train_esd(prompt, negative_prompt, p_unlearn, model_path,
    ...
    name = f'compvis-word_{word_print}'
    model_metadata = {
        'negative_prompt': negative_word_print,
        'p_unlearn': p_unlearn,
    ...
    for i in pbar:
        ...
        if (i + 1) % 500 == 0:
            save_model(model, model_path, name, i - 1, model_metadata, save_compvis=True, save_diffusers=True)
        if i % 100 == 0:
            save_history(losses, model_path, name, word_print)

    model.eval()

    save_model(model, model_path, name, None, model_metadata, save_compvis=True, save_diffusers=True, compvis_config_file=config_path, diffusers_config_file=diffusers_config_path)
    save_history(losses, model_path, name, word_print)

def save_model(model, model_path, name, num, metadata, compvis_config_file=None, diffusers_config_file=None, device='cpu', save_compvis=True, save_diffusers=True):
    # SAVE MODEL
